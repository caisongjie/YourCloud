...
public class PowerMonitor {
	...
	private ArrayList<double[]> hostsPower = new ArrayList<double[]>();
	private ArrayList<double[]> hostPowerAverage = new ArrayList<double[]>();
	...
	public void hostDetails(int hostId, double power, double maxPower,
			double utilization, double time, double datacenterPower, int hostNum) {
		...
		double sad[] = { time, tempHostPowerAverage / hostNum,
				tempUtilPowerAverage / hostNum };
		hostPowerAverage.add(sad);

		hostNumber = hostNum;
		double hsad[] = { hostId, power, maxPower, utilization, time };
		hostsPower.add(hsad);
		currentTime = time;
