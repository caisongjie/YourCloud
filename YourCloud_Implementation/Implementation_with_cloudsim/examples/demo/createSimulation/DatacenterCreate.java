...
import demo.files.PowerModuleMinimumUtil;

public class DatacenterCreate {

	public static PowerDatacenterExtra createDatacenter(String name,
		...
		for (int i = 0; i < Constant.HOST_TYPES; i++) {
			for (int host = 0; host < Constant.VM_HOST_NUMBERS[i]; host++) {
				...
				hostList.add(new PowerHostExtra(
						id,
		...
		try {
			...
			Class<?> allocationClass = Class
					.forName("demo.vmSchedulers." + allocationClassName);
			...
			datacenter = new PowerDatacenterExtra(name.replaceAll(" ", "_"),
